    ...
    BEGIN
	 DECLARE
        ...
        BEGIN
			 ...
			 into  ncac
			 from cchvalue   
			 where dosidprospect=P_DOSID 
			 AND cchsid='TFDCCHSID91' ;
			 
			   If ( ncac >= 10000001 ) Then 
...
PROCEDURE P_SGMDTLIM (P_LANCODE     LANPROCESS.LANCODE%TYPE,        --ABH 07/04/2021  La date d’expiration d’une enveloppe est égale à la date d’effet de l’enveloppe incrémentée de la durée de validité moins un jour ;
                              P_DOSID       DOSSIERPROSPECT.DOSID%TYPE,
                              DPRVERSION    DOSSIERPROSPECT.DPRVERSION%TYPE,
                              P_PFIID       DPRPROPFINANCE.PFIID%TYPE,
                              P_ACTID       ACTEUR.ACTID%TYPE,
        ...
        to_char(extract (month from max( WSTDTEXEC)))||'/' || 
           ...
           and worcode||wstorder 
             in (select worcode||wstorder from lanworstep where worcode in 
              ('WFCLIPRO','WFCLICOM')and lancode='FR' and wstlabel like '%écision%') ; */
